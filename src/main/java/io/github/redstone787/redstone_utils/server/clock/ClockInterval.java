...
public record ClockInterval(int ticks) {
    ...
    public static ParseResult parse(String value) {
        ...
        if (ticks < MIN_COMPARATOR_TICKS || ticks % COMPARATOR_TICK_STEP != 0) {
            return ParseResult.failure(Component.translatable("message.redstone_utils.clock.comparator_range"));
        }
    ...
    private static ParseResult parseValue(String value, int maximumTicks) {
        ...
        if (!matcher.matches()) {
            return ParseResult.failure(Component.translatable("message.redstone_utils.clock.invalid_interval"));
        }
        ...
        try {
            ...
            return ParseResult.failure(Component.translatable("message.redstone_utils.clock.too_large"));
        }

        if (amount.signum() <= 0) {
            return ParseResult.failure(Component.translatable("message.redstone_utils.clock.positive"));
        }
        ...
        if (ticksValue.compareTo(BigDecimal.valueOf(maximumTicks)) > 0) {
            return ParseResult.failure(Component.translatable(
                    "message.redstone_utils.clock.maximum",
                    maximumTicks,
        ...
        try {
            ...
            return ParseResult.failure(Component.translatable(seconds
                    ? "message.redstone_utils.clock.whole_seconds"
                    : "message.redstone_utils.clock.whole_ticks"));
        }
