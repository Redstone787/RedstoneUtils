    ...
    public static Expansion expand(String command) {
        ...
        for (int depth = 0; ; depth++) {
            ...
            if (depth >= MAX_ALIAS_DEPTH) {
                return new Expansion(currentCommand, expanded, Component.translatable("message.redstone_utils.macro.alias_depth").getString());
            }

            if (!visitedAliases.add(root)) {
                return new Expansion(currentCommand, expanded, Component.translatable("message.redstone_utils.macro.alias_loop", "/" + root).getString());
            }
