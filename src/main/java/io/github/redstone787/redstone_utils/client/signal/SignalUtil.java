...
public final class SignalUtil {
    ...
    public static void init() {
        ...
        ClientCommandRegistrationCallback.EVENT.register(((dispatcher, buildContext) -> {
            ...
            dispatcher.register(ClientCommands.literal("redstone_utils").then(signalCommand(buildContext)));
    ...
    private static int giveSignalStrength(FabricClientCommandSource source, int strength) {
        ...
        source.sendFeedback(Component.translatable("message.redstone_utils.signal.sent_barrel", strength));
    ...
    private static int giveOptimalSignalStrength(FabricClientCommandSource source, int strength) {
        ...
        source.getPlayer().connection.sendCommand(command);
        source.sendFeedback(Component.translatable("message.redstone_utils.signal.sent_optimal", strength));
    ...
    private static int createSignalBlock(FabricClientCommandSource source, int strength, String blockName) {
        ...
        if (variant == null) {
            source.sendFeedback(Component.translatable("message.redstone_utils.signal.unknown", SignalBlockVariant.suggestions()));
            return 0;
        }
        ...
        source.getPlayer().connection.sendCommand(command);
        source.sendFeedback(Component.translatable("message.redstone_utils.signal.sent_give", variant.displayName(), strength));
        return 1;
    }
    ...
    private static int setTargetedSignalBlock(FabricClientCommandSource source, int strength, SignalBlockVariant variant, String blockArgument) {
        ...
        target.source().sendFeedback(Component.translatable("message.redstone_utils.signal.sent_setblock", variant.displayName(), strength));
        return 1;
    }

    private static int showSignalBlockOptions(FabricClientCommandSource source) {
        source.sendFeedback(Component.translatable("message.redstone_utils.signal.usage"));
        source.sendFeedback(Component.translatable("message.redstone_utils.signal.supported", SignalBlockVariant.suggestions()));
        return 1;
    }
    ...
    private static int setContent(
        ...
        return setContainerContent(target, actualAmount, Component.translatable(
                "message.redstone_utils.signal.sent_content",
                actualAmount
        ));
    }
    ...
    private static int setSignal(
        ...
        return setContainerContent(target, amount, Component.translatable("message.redstone_utils.signal.sent_signal", strength, amount));
    }
