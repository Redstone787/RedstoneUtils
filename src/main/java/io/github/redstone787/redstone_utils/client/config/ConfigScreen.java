...
import io.github.redstone787.redstone_utils.client.ui.RedstoneUi;
...
public final class ConfigScreen extends Screen {
    ...
    public ConfigScreen() {
        super(Component.translatable("screen.redstone_utils.config"));
    }
    ...
    protected void init() {
        ...
        searchBox = addWidget(new EditBox(
                ...
                layout.searchWidth(),
                22,
                Component.translatable("config.redstone_utils.search")
        ));
        searchBox.setHint(Component.translatable("config.redstone_utils.search"));
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTicks) {
        ...
        graphics.text(font, Component.translatable("screen.redstone_utils.config.profile", RedstoneUtilsConfig.activeProfile()), layout.x + 14, layout.y + 27, RedstoneUi.MUTED_TEXT_COLOR, false);
        ...
        for (int index = 0; index < visible.size(); index++) {
            ...
            if (valueHovered) {
                ...
                graphics.setTooltipForNextFrame(Component.translatable("config.redstone_utils.reset_one"), mouseX, mouseY);
            }
        }
        ...
        drawButton(graphics, Component.translatable("config.redstone_utils.reset_profile").getString(), layout.x + 14, footerY, 132, 24,
    ...
    private List<Option> createOptions() {
        ...
        result.add(toggle(Category.GENERAL, "config.redstone_utils.hud", RedstoneUtilsConfig::isHudOverlayVisible, RedstoneOverlay::setVisible, true));
        result.add(toggle(Category.GENERAL, "config.redstone_utils.status_hud", RedstoneUtilsConfig::isStatusHudVisible, RedstoneUtilsConfig::setStatusHudVisible, true));
        result.add(choice(Category.GENERAL, "config.redstone_utils.status_anchor", RedstoneUtilsConfig.HudAnchor.values(), RedstoneUtilsConfig::getStatusHudAnchor, RedstoneUtilsConfig::setStatusHudAnchor, RedstoneUtilsConfig.HudAnchor.TOP_RIGHT));
        result.add(choice(Category.GENERAL, "config.redstone_utils.feedback", RedstoneMessages.MessageTarget.values(), RedstoneMessages::getDefaultTarget, RedstoneMessages::setDefaultTarget, RedstoneMessages.MessageTarget.POPUP));
        result.add(number(Category.GENERAL, "config.redstone_utils.teleport", 10, 1000, 0, RedstoneUtilsConfig::getTeleportMaxRange, RedstoneUtilsConfig::setTeleportMaxRange, 100));

        result.add(choice(Category.AUTOWIRE, "config.redstone_utils.autowire", WireType.values(), AutoWireHandler::getActiveWireType, AutoWireHandler::setActiveWireType, WireType.NONE));
        result.add(toggle(Category.AUTOWIRE, "config.redstone_utils.wire_preview", AutoWirePreviewOverlay::isVisible, AutoWirePreviewOverlay::setVisible, true));

        result.add(toggle(Category.OVERLAYS, "config.redstone_utils.bud", BudSwitchOverlay::isVisible, BudSwitchOverlay::setVisible, true));
        result.add(number(Category.OVERLAYS, "config.redstone_utils.bud_range", 4, 64, 0, () -> (double) RedstoneUtilsConfig.getBudTestRange(), value -> { RedstoneUtilsConfig.setBudTestRange(value.intValue()); BudSwitchOverlay.clear(); }, 8));
        result.add(toggle(Category.OVERLAYS, "config.redstone_utils.sculk", SculkSensorOverlay::isVisible, SculkSensorOverlay::setVisible, false));
        result.add(number(Category.OVERLAYS, "config.redstone_utils.sculk_distance", 16, 256, 0, () -> (double) RedstoneUtilsConfig.getSculkSensorSearchDistance(), value -> { RedstoneUtilsConfig.setSculkSensorSearchDistance(value.intValue()); SculkSensorOverlay.requestRefresh(); }, 96));
        result.add(toggle(Category.OVERLAYS, "config.redstone_utils.freeze", OverlayFreeze::anyFrozen, OverlayFreeze::setAllFrozen, false));

        result.add(choice(Category.ACCESSIBILITY, "config.redstone_utils.palette", RedstoneUtilsConfig.ColorPalette.values(), RedstoneUtilsConfig::getColorPalette, RedstoneUtilsConfig::setColorPalette, RedstoneUtilsConfig.ColorPalette.DEFAULT));
        result.add(color(Category.ACCESSIBILITY, "config.redstone_utils.wire_color", RedstoneUtilsConfig::wirePreviewColor, RedstoneUtilsConfig::setWirePreviewColor, RedstoneUtilsConfig::resetWirePreviewColor));
        result.add(color(Category.ACCESSIBILITY, "config.redstone_utils.risk_color", RedstoneUtilsConfig::budRiskColor, RedstoneUtilsConfig::setBudRiskColor, RedstoneUtilsConfig::resetBudRiskColor));
        result.add(color(Category.ACCESSIBILITY, "config.redstone_utils.source_color", RedstoneUtilsConfig::budSourceColor, RedstoneUtilsConfig::setBudSourceColor, RedstoneUtilsConfig::resetBudSourceColor));
        result.add(color(Category.ACCESSIBILITY, "config.redstone_utils.sculk_color", RedstoneUtilsConfig::sculkColor, RedstoneUtilsConfig::setSculkColor, RedstoneUtilsConfig::resetSculkColor));
        result.add(number(Category.ACCESSIBILITY, "config.redstone_utils.opacity", 0.1, 1.0, 2, () -> (double) RedstoneUtilsConfig.getOverlayOpacity(), value -> RedstoneUtilsConfig.setOverlayOpacity(value.floatValue()), 0.85));
        result.add(number(Category.ACCESSIBILITY, "config.redstone_utils.line_width", 1, 8, 1, () -> (double) RedstoneUtilsConfig.getOverlayLineWidth(), value -> RedstoneUtilsConfig.setOverlayLineWidth(value.floatValue()), 2));
        result.add(toggle(Category.ACCESSIBILITY, "config.redstone_utils.through_walls", RedstoneUtilsConfig::renderOverlaysThroughWalls, RedstoneUtilsConfig::setOverlayThroughWalls, false));
    ...
    private static Option toggle(Category category, String key, Supplier<Boolean> getter, Consumer<Boolean> setter, boolean defaultValue) {
        return new Option(category, key + ".title", key + ".description", key + ".tooltip",
                () -> Component.translatable(getter.get() ? "state.redstone_utils.on" : "state.redstone_utils.off").getString(),
    ...
    private static String enumLabel(Object value) {
        ...
        return Component.translatable("enum.redstone_utils." + id).getString();
    }

    private enum Category {
        ALL, GENERAL, AUTOWIRE, OVERLAYS, ACCESSIBILITY, PERFORMANCE;
        private String label() { return Component.translatable("config.redstone_utils.category." + name().toLowerCase(Locale.ROOT)).getString(); }
    }
