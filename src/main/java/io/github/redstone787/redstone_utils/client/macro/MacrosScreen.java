...
public final class MacrosScreen extends Screen {
    ...
    public MacrosScreen() {
        super(Component.translatable("screen.redstone_utils.macros"));
    }
    ...
    protected void init() {
        ...
        searchBox = addWidget(new EditBox(font, layout.searchX(), layout.searchY(), layout.searchWidth(), 22,
                Component.translatable("macros.redstone_utils.search")));
        searchBox.setHint(Component.translatable("macros.redstone_utils.search"));
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTicks) {
        ...
        graphics.text(font, Component.translatable("screen.redstone_utils.macros.profile", MacroStore.activeProfile()), layout.x + 14, layout.y + 26, RedstoneUi.MUTED_TEXT_COLOR, false);
        ...
        drawButton(graphics, Component.translatable("macros.redstone_utils.sort", sortOrder.label()).getString(), layout.sortX(), layout.controlsY(), layout.sortWidth(), 22,
                RedstoneUi.contains(mouseX, mouseY, layout.sortX(), layout.controlsY(), layout.sortWidth(), 22), RedstoneUi.ButtonTone.NORMAL);
        drawButton(graphics, Component.translatable("macros.redstone_utils.import").getString(), layout.importX(), layout.controlsY(), 74, 22,
                RedstoneUi.contains(mouseX, mouseY, layout.importX(), layout.controlsY(), 74, 22), RedstoneUi.ButtonTone.NORMAL);
        drawButton(graphics, Component.translatable("macros.redstone_utils.export").getString(), layout.exportX(), layout.controlsY(), 78, 22,
        ...
        if (macros.isEmpty()) {
            graphics.text(font, Component.translatable("macros.redstone_utils.empty"), layout.left() + 10, layout.top() + 16, RedstoneUi.MUTED_TEXT_COLOR, false);
        }
    ...
    private void drawRow(GuiGraphicsExtractor graphics, Layout layout, Macro macro, int index, int mouseX, int mouseY) {
        ...
        String[] labels = {
                Component.translatable(macro.enabled() ? "macros.redstone_utils.disable" : "macros.redstone_utils.enable").getString(),
                Component.translatable("macros.redstone_utils.edit").getString(),
                Component.translatable("macros.redstone_utils.copy").getString(),
                Component.translatable("macros.redstone_utils.delete").getString()
        };
    ...
    private void drawFooter(GuiGraphicsExtractor graphics, Layout layout, int mouseX, int mouseY) {
        ...
        footerButton(graphics, "macros.redstone_utils.new_keybind", layout.footerKeyX(), y, layout.footerKeyWidth(), mouseX, mouseY);
        footerButton(graphics, "macros.redstone_utils.new_command", layout.footerCommandX(), y, layout.footerCommandWidth(), mouseX, mouseY);
    ...
    private void confirmDelete(Macro macro) {
        minecraft.gui.setScreen(new ConfirmScreen(confirmed -> {
            ...
            minecraft.gui.setScreen(this);
                ...
                Component.translatable("macros.redstone_utils.delete_confirm.message", macro.name())));
    }

    private void importMacros() {
        try {
            int count = MacroStore.importIntoActiveProfile(null);
            RedstoneMessages.popup(Component.translatable("macros.redstone_utils.imported", count, MacroStore.defaultExportPath().toString()));
        } catch (IOException exception) {
            RedstoneMessages.popup(Component.translatable("macros.redstone_utils.import_failed", MacroStore.defaultExportPath().toString()));
        }
    }
    ...
    private void exportMacros() {
        try {
            ...
            RedstoneMessages.popup(Component.translatable("macros.redstone_utils.exported", count, MacroStore.defaultExportPath().toString()));
        } catch (IOException exception) {
            RedstoneMessages.popup(Component.translatable("macros.redstone_utils.export_failed", MacroStore.defaultExportPath().toString()));
        }
    }
    ...
    private enum SortOrder {
        CATEGORY, NAME, TYPE;
        Component label() { return Component.translatable("macros.redstone_utils.sort." + name().toLowerCase(Locale.ROOT)); }
