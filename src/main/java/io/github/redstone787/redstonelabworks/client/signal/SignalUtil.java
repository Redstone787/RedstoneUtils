...
public final class SignalUtil {
    ...
    public static void init() {
        ...
        ClientCommandRegistrationCallback.EVENT.register(((dispatcher, buildContext) -> {
            ...
            dispatcher.register(ClientCommands.literal("redstonelabworks").then(signalCommand(buildContext)));
    ...
    private static boolean hasServerBackend() {
        return RedstoneLabworksClientNetworking.hasServerBackend();
    }

    private static int forwardToServer(CommandContext<FabricClientCommandSource> context) {
        return RedstoneLabworksClientNetworking.sendServerCommand(context.getInput()) ? 1 : 0;
    }
    ...
    private static int giveSignalStrength(FabricClientCommandSource source, int strength) {
        ...
        source.sendFeedback(Component.translatable("message.redstonelabworks.signal.sent_barrel", strength));
    ...
    private static int giveOptimalSignalStrength(FabricClientCommandSource source, int strength) {
        ...
        source.getPlayer().connection.sendCommand(command);
        source.sendFeedback(Component.translatable("message.redstonelabworks.signal.sent_optimal", strength));
    ...
    private static int createSignalBlock(FabricClientCommandSource source, int strength, String blockName) {
        ...
        if (variant == null) {
            source.sendFeedback(Component.translatable("message.redstonelabworks.signal.unknown", SignalBlockVariant.suggestions()));
            return 0;
        }
        ...
        source.getPlayer().connection.sendCommand(command);
        source.sendFeedback(Component.translatable("message.redstonelabworks.signal.sent_give", variant.displayName(), strength));
        return 1;
    }
    ...
    private static int setTargetedSignalBlock(FabricClientCommandSource source, int strength, SignalBlockVariant variant, String blockArgument) {
        ...
        target.source().sendFeedback(Component.translatable("message.redstonelabworks.signal.sent_setblock", variant.displayName(), strength));
        return 1;
    }

    private static int showSignalBlockOptions(FabricClientCommandSource source) {
        source.sendFeedback(Component.translatable("message.redstonelabworks.signal.usage"));
        source.sendFeedback(Component.translatable("message.redstonelabworks.signal.supported", SignalBlockVariant.suggestions()));
        return 1;
    }
    ...
    private static int setContent(
        ...
        return setContainerContent(target, actualAmount, Component.translatable(
                "message.redstonelabworks.signal.sent_content",
                actualAmount
        ));
    }
    ...
    private static int setSignal(
        ...
        return setContainerContent(target, amount, Component.translatable("message.redstonelabworks.signal.sent_signal", strength, amount));
    }
