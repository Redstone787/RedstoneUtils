...
public class AutoWireHandler {
    ...
    private static WireType activeWireType = RedstoneLabworksConfig.getActiveWireType();
    ...
    public static void setActiveWireType(WireType wireType) {
        ...
        RedstoneLabworksConfig.setActiveWireType(activeWireType);
        AutoWire.reset();
        RedstoneLabworksClientNetworking.setServerAutoWire(activeWireType);
        RedstoneMessages.popup(net.minecraft.network.chat.Component.translatable(
                "message.redstonelabworks.autowire.active",
    ...
    public static void applyServerMode(String mode, boolean accepted) {
        ...
        RedstoneLabworksConfig.setActiveWireType(serverMode);
        AutoWire.reset();
        if (!accepted) {
            RedstoneMessages.popup(net.minecraft.network.chat.Component.translatable(
                    "message.redstonelabworks.autowire.rejected",
    ...
    public static void reloadFromProfile() {
        activeWireType = RedstoneLabworksConfig.getActiveWireType();
        AutoWire.reset();
        syncServerMode();
    }
    ...
    private static InteractionResult onUseBlock(Player player, Level level, InteractionHand hand, BlockHitResult hitResult) {
        ...
        if (RedstoneLabworksClientNetworking.hasAutoWireBackend()) return InteractionResult.PASS;
