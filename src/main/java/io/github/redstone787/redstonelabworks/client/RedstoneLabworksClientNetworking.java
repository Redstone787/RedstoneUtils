...
public final class RedstoneLabworksClientNetworking {

    private RedstoneLabworksClientNetworking() {
    }

    public static void init() {
        ClientPlayNetworking.registerGlobalReceiver(RedstoneLabworksNetworking.ClientCommandPayload.TYPE, (payload, context) ->
                context.client().execute(() -> RedstoneLabworksClientActions.run(payload.action(), payload.value()))
        );
        ClientPlayNetworking.registerGlobalReceiver(RedstoneLabworksNetworking.AutoWireFeedbackPayload.TYPE, (payload, context) ->
                context.client().execute(() -> RedstoneMessages.popup(Component.translatable(
                        "message.redstonelabworks.autowire.failure." + payload.reason(),
                        Component.translatable(payload.componentTranslationKey())
                )))
        );
        ClientPlayNetworking.registerGlobalReceiver(RedstoneLabworksNetworking.AutoWireStatePayload.TYPE, (payload, context) ->
    ...
    public static boolean hasServerBackend() {
        return canSend(RedstoneLabworksNetworking.BackendProbePayload.TYPE);
    }

    public static boolean hasAutoWireBackend() {
        return canSend(RedstoneLabworksNetworking.SetAutoWirePayload.TYPE);
    }

    public static boolean hasTeleportBackend() {
        return canSend(RedstoneLabworksNetworking.TeleportPayload.TYPE);
    }
    ...
    public static boolean setServerAutoWire(WireType wireType) {
        ...
        ClientPlayNetworking.send(new RedstoneLabworksNetworking.SetAutoWirePayload(mode));
        return true;
    }
    ...
    public static boolean teleport(double range) {
        ...
        ClientPlayNetworking.send(new RedstoneLabworksNetworking.TeleportPayload(range));
        return true;
    }
