...
final class MacroEditScreen extends Screen {
    ...
    private MacroEditScreen(Screen parent, Macro macro, MacroType fallbackType) {
        super(Component.translatable("screen.redstonelabworks.macro_edit"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        nameBox = addWidget(createBox(text("macro.redstonelabworks.name"), nameValue, Macro.MAX_NAME_LENGTH));
        commandBox = addWidget(createBox(text("macro.redstonelabworks.command"), commandValue, Macro.MAX_COMMAND_LENGTH));
        aliasBox = addWidget(createBox(text("macro.redstonelabworks.alias"), aliasValue, Macro.MAX_ALIAS_LENGTH));
        categoryBox = addWidget(createBox(text("macro.redstonelabworks.category"), categoryValue, 48));
        ...
        if (editing && RedstoneUi.contains(mouseX, mouseY, layout.deleteX(), layout.footerButtonY(), layout.footerButtonWidth(), BUTTON_HEIGHT)) {
            ...
            Minecraft.getInstance().gui.setScreen(new ConfirmScreen(confirmed -> {
                ...
                Minecraft.getInstance().gui.setScreen(confirmed ? parent : this);
                    ...
                    Component.translatable("macros.redstonelabworks.delete_confirm.message", nameValue)));
            playClick();
            return true;
        }
    ...
    private void drawHeader(GuiGraphicsExtractor graphics, Layout layout) {
        String title = text(editing ? "macro.redstonelabworks.edit_title" : "macro.redstonelabworks.new_title");
        ...
        String subtitle = error.isBlank()
                ? text("macro.redstonelabworks.subtitle")
                : error;
    ...
    private void drawTypeSelector(GuiGraphicsExtractor graphics, Layout layout, int mouseX, int mouseY) {
        ...
        RedstoneUi.drawButton(graphics, font, text("macro.redstonelabworks.keybind"), layout.keybindTypeX(), layout.typeY(), layout.typeButtonWidth(), BUTTON_HEIGHT, keybindHovered, type == MacroType.KEYBIND ? RedstoneUi.ButtonTone.ACTIVE : RedstoneUi.ButtonTone.NORMAL);
        RedstoneUi.drawButton(graphics, font, text("macro.redstonelabworks.command_type"), layout.commandTypeX(), layout.typeY(), layout.typeButtonWidth(), BUTTON_HEIGHT, commandHovered, type == MacroType.COMMAND ? RedstoneUi.ButtonTone.ACTIVE : RedstoneUi.ButtonTone.NORMAL);
    }

    private void drawFields(GuiGraphicsExtractor graphics, Layout layout, int mouseX, int mouseY, float deltaTicks) {
        drawLabel(graphics, text("macro.redstonelabworks.name"), layout.nameX(), layout.nameLabelY());
        nameBox.extractRenderState(graphics, mouseX, mouseY, deltaTicks);

        drawLabel(graphics, text("macro.redstonelabworks.command_to_run"), layout.commandX(), layout.commandLabelY());
        commandBox.extractRenderState(graphics, mouseX, mouseY, deltaTicks);

        if (type == MacroType.COMMAND) {
            drawLabel(graphics, text("macro.redstonelabworks.alias"), layout.bindingX(), layout.bindingLabelY());
            aliasBox.extractRenderState(graphics, mouseX, mouseY, deltaTicks);
            if (layout.showHint()) {
                RedstoneUi.drawFittedText(graphics, font, text("macro.redstonelabworks.alias_hint"), layout.bindingX(),
                        layout.hintY(), layout.columnWidth(), RedstoneUi.MUTED_TEXT_COLOR);
            }
        } else {
            drawLabel(graphics, text("macro.redstonelabworks.binding"), layout.bindingX(), layout.bindingLabelY());
            boolean hovered = RedstoneUi.contains(mouseX, mouseY, layout.bindingX(), layout.bindingFieldY(), layout.columnWidth(), FIELD_HEIGHT);
            String label = capturingKey ? text("macro.redstonelabworks.capture") : MacroKeys.displayName(keyCode, mouseButton, modifiers);
            ...
            if (layout.showHint()) {
                RedstoneUi.drawFittedText(graphics, font, text("macro.redstonelabworks.capture_hint"), layout.bindingX(),
                        layout.hintY(), layout.columnWidth(), RedstoneUi.MUTED_TEXT_COLOR);
            }
        }

        drawLabel(graphics, text("macro.redstonelabworks.category"), layout.categoryX(), layout.categoryLabelY());
        ...
        if (type == MacroType.KEYBIND) {
            ...
            RedstoneUi.drawButton(graphics, font, Component.translatable("macro.redstonelabworks.trigger", text("enum.redstonelabworks." + trigger.name().toLowerCase(java.util.Locale.ROOT))).getString(), layout.triggerX(), layout.triggerY(), layout.columnWidth(), BUTTON_HEIGHT, triggerHovered, RedstoneUi.ButtonTone.NORMAL);
        }
        ...
        RedstoneUi.drawButton(graphics, font, text(enabled ? "state.redstonelabworks.enabled" : "state.redstonelabworks.disabled"), layout.enabledX(), layout.triggerY(), layout.columnWidth(), BUTTON_HEIGHT, enabledHovered,
    ...
    private void drawFooter(GuiGraphicsExtractor graphics, Layout layout, int mouseX, int mouseY) {
        ...
        if (editing) {
            RedstoneUi.drawButton(graphics, font, text("macros.redstonelabworks.delete"), layout.deleteX(), layout.footerButtonY(), layout.footerButtonWidth(), BUTTON_HEIGHT, deleteHovered, RedstoneUi.ButtonTone.DANGER);
        }
        ...
        RedstoneUi.drawButton(graphics, font, text("macro.redstonelabworks.save"), layout.saveX(), layout.footerButtonY(), layout.footerButtonWidth(), BUTTON_HEIGHT, saveHovered, RedstoneUi.ButtonTone.ACTIVE);
    }
    ...
    private String validate() {
        ...
        if (nameValue.isBlank()) return text("macro.redstonelabworks.error.name");
        if (commandValue.isBlank()) return text("macro.redstonelabworks.error.command");

        if (type == MacroType.KEYBIND) {
            if (!MacroKeys.isBound(keyCode)) return text("macro.redstonelabworks.error.binding");
            if (MacroStore.bindingExists(keyCode, mouseButton, modifiers, originalId)) return text("macro.redstonelabworks.error.binding_used");
            return "";
        }

        if (!MacroCommandText.isValidAliasInput(aliasBox.getValue())) return text("macro.redstonelabworks.error.alias");
        if (CommandCommand.isReservedAlias(aliasValue)) return Component.translatable("macro.redstonelabworks.error.reserved", "/" + aliasValue).getString();
        if (MacroCommandText.normalizeAlias(commandValue).equals(aliasValue)) return text("macro.redstonelabworks.error.self");
        if (MacroStore.aliasExists(aliasValue, originalId)) return text("macro.redstonelabworks.error.alias_used");
