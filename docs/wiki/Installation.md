...
3. Add the Redstone Utils JAR to the same directory.
4. For the complete multiplayer feature set, repeat steps 2 and 3 on the server.
5. Start the game/server and confirm that Fabric lists `redstone_utils` without dependency errors.
...
```shell
git clone https://github.com/Redstone787/RedstoneUtils.git
cd RedstoneUtils
./gradlew build
```
